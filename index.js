...
app.all("/", async function (req, res) {
  const message = {
    status: "ok",
    message:
      "You have reached the OpenAI API. To get started, visit https://platform.openai.com",
  };
...
if (SSL_ENABLED) {
  ...
  httpsServer.listen(SERVER_PORT, () => {
    ...
    console.event(
      "SRV_START",
      `HTTPS Server is listening on port ${SERVER_PORT}`,
    );
  });
} else {
  ...
  httpServer.listen(SERVER_PORT, () => {
    ...
    console.event(
      "SRV_START",
      `HTTP Server is listening on port ${SERVER_PORT}`,
    );
  });
}
