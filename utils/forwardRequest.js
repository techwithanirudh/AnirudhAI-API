...
import { OPENAI_CONFIG, ERROR_MESSAGE_ALL_PROXIES_DOWN, ERROR_MESSAGE_GENERIC, ERROR_DURATION, ERROR_COUNT_THRESHOLD } from "../config/index.js";
...
function addErrorTimestamp() {
	...
	errorTimestamps = errorTimestamps.filter(timestamp => now - timestamp <= ERROR_DURATION);

	if (errorTimestamps.length > ERROR_COUNT_THRESHOLD) {
		console.event('REFETCHING_IN', `1s as API Error was triggered more than ${ERROR_COUNT_THRESHOLD} times within ${ERROR_DURATION / 1000} seconds, refetching`);
		updateCurrentProxy();
